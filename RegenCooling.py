...
class HeatTransfer():
	def __init__(self, cea, gas, geometry, material, coolant, cooling_geometry, m_dot, m_dot_coolant, model='standard-bartz', cool_model='gnielinski', eta_combustion=0.92):
		...
		self.t_wall = cooling_geometry.t_wall	        # wall thickness [m]
		...
		self.T_wall_arr     = np.ndarray(len(self.geometry[:,1]))
	...
	def heat_flux(self, idx, maxIter=100, tol=1e-6):
		T_wall_guess = 600
		T_wall_coolant = 400
		Niter = 0
		diff = 1
		...
		while diff > tol:
			...
			self.q      = (self.T_hg - self.coolant.T + self.q_rad/self.halpha) / (1/self.halpha + self.t_wall[idx]/self.material.k(T_wall_guess) + 1/self.halpha_c)
			self.T_wall = -((self.q - self.q_rad)/self.halpha - self.T_hg)
			
			T_wall_coolant = -self.q * self.t_wall[idx] / self.material.k(T_wall_guess) + T_wall_guess
	...
	def run_sim(self):
		...
		for i in range(len(self.geometry[:,1])):
			...
			self.T_wall_arr[idx]   = self.T_wall
