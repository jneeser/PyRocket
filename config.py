...
# CEA input
fuel            = 'C2H6'                             # choose existing fuel or oxidiser from rocketcea or create new fuel or oxidiser blend in PropLibrary
ox              = 'N2O'
...
MR              = 6.83
m_dot           = 6.81e-3                            # total mass flow [kg/s]
m_dot_f         = m_dot / (MR + 1)
m_dot_ox        = m_dot - m_dot_f
Pc              = 8.1e5                              # Chamber pressure [Pa]
...
inlet_temp      = 294.65                     		 # inlet temperature [K]
...
# Chamber geometry
D_c       = 14e-3                                    # chamber diamter [m]
D_t       = 4.2e-3                                   # throat diameter [m]
D_e       = 5.2e-3                                   # exit diamter [m]
L_cyl     = 65e-3                                    # cylindrical chamber length [m]
r_1       = 12.03e-3                                 # converging section radius [m]
r_2       = 3e-3                                     # throat converging section radius [m]
r_n       = 3e-3                                     # throat diverging radius [m]
phi_conv  = 30                                       # convergence angle [deg]
phi_div   = 15                                       # divergence angle [deg]
phi_e     = 15                                       # exit angle [deg]
step_size = 0.0012                                   # step size along the chamber contour [m]
...
psi   = 1/3                                          # fill factor of the cooling channels; fraction of the circumferecne covered by the cooling channels (0 - 1) CAN BE FLOAT OR FUNCTION
...
max_iter     = 1000									 # maximum number of iterations before termination
...
log_TC = True                                        # temperature at thermocouple locations is to be logged
...
save_path = "SectionImages597_Z_23"
